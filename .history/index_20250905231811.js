...
app.post("/finalOrder", async (req, res) => {
      try {
        ...
        const orderData = {
          ...
          paidstatus: "pending",
          createdAt: new Date(),
          TranjectionId: result.insertedId.toString()
        };

        const result = await FinalorderInfoCollaction.insertOne(orderData);

        // SSLCommerz Payment Data
        const data = {
          total_amount: total,
          currency: "BDT",
          tran_id: result.insertedId.toString(), // unique transaction id
          success_url:`http://localhost:5000/payment/success/${result.insertedId.toString()}`,
...
app.post("/payment/success/:result.insertedId.toString()",async(req,res)=>{
  console.log(req.params.result.insertedId.toString());
  const result =await FinalorderInfoCollaction.updateOne({TranjectionId:req.params.tran_id},{
    $set:{paidstatus: true}
  })
...
app.post('/payment/fail/:result.insertedId.toString()',async(req,res)=>{
  const result = FinalorderInfoCollaction.deleteOne({TranjectionId:req.params.tran_id})
  if(res.deletedCount){
