...
const orderIds = orders.map(order => new ObjectId(order._id));
    ...
    const data = {
      ...
      cancel_url: `http://localhost:5173/payment/cancel/${tran_id}`,
      ipn_url: "http://localhost:5173/payment/ipn",
...
app.post("/payment/success/:tran_id", async (req, res) => {
  ...
  if (result.modifiedCount > 0) {
    res.redirect(`http://localhost:5000/payment/success/${tran_id}`);
  } else {
...
app.post("/payment/fail/:tran_id", async (req, res) => {
  ...
  if (result.deletedCount > 0) {
    res.redirect(`http://localhost:5000/payment/fail/${tran_id}`);
  } else {
...
app.post("/payment/cancel/:tran_id", async (req, res) => {
  ...
  if (result.deletedCount > 0) {
    res.redirect(`http://localhost:5000/payment/cancel/${tran_id}`);
  } else {
    res.status(400).send({ message: "Transaction not found to delete" });
  }
});
